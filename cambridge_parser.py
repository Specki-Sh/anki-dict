...
def get_tags(
    tags_section: Optional[bs4.Tag],
) -> tuple[LEVEL_T, LABELS_AND_CODES_T, REGIONS_T, USAGES_T, DOMAINS_T]:
...
def update_word_dict(
    ...
    last_appended_data["definitions_translations"].append(
        remove_blanks_from_str(definition_translation)
        if definition_translation is not None
        else ""
    )
    last_appended_data["levels"].append(
        remove_blanks_from_str(level) if level is not None else ""
    )
    last_appended_data["image_links"].append(
        remove_blanks_from_str(image_link) if image_link is not None else ""
    )
    ...
    last_appended_data["examples_translations"].append(
        remove_blanks_from_list(examples_translations)
        if examples_translations is not None
        else []
    )
    last_appended_data["alt_terms"].append(
        remove_blanks_from_list(alt_terms) if alt_terms is not None else []
    )
    last_appended_data["irregular_forms"].append(
        remove_blanks_from_list(irregular_forms) if irregular_forms is not None else []
    )
    last_appended_data["labels_and_codes"].append(
        remove_blanks_from_list(labels_and_codes)
        if labels_and_codes is not None
        else []
    )
    last_appended_data["regions"].append(
        remove_blanks_from_list(regions) if regions is not None else []
    )
    last_appended_data["usages"].append(
        remove_blanks_from_list(usages) if usages is not None else []
    )
    last_appended_data["domains"].append(
        remove_blanks_from_list(domains) if domains is not None else []
    )
...
def get_irregular_forms(word_header_block: Optional[bs4.Tag]) -> IRREGULAR_FORMS_T:
    ...
    all_irreg_forms_block = word_header_block.find(
        "span", {"class": "irreg-infls dinfls"}
    )
...
def get_alt_terms(word_header_block: Optional[bs4.Tag]) -> ALT_TERMS_T:
    ...
    var_block.extend(
        word_header_block.find_all("span", {"class": "spellvar dspellvar"})
    )
...
def define(
    ...
    for dictionary_index in range(len(primal_block)):
        ...
        main_block = primal_block[dictionary_index].find_all(
            "div",
            {"class": lambda x: "entry-body__el" in x if x is not None else False},
        )
        main_block.extend(
            primal_block[dictionary_index].find_all("div", {"class": "pr dictionary"})
        )
        main_block.extend(
            primal_block[dictionary_index].find_all("div", {"class": "pv-block"})
        )
        main_block.extend(
            primal_block[dictionary_index].find_all("div", {"class": "pr idiom-block"})
        )
        ...
        for entity in main_block:
            ...
            if parsed_word_block is None:
                parsed_word_block = (
                    header_block.find("h2", {"class": "di-title"})
                    if header_block is not None
                    else None
                )
            if parsed_word_block is None:
                parsed_word_block = (
                    header_block.find("div", {"class": "di-title"})
                    if header_block is not None
                    else None
                )
            header_word = (
                parsed_word_block.text if parsed_word_block is not None else ""
            )

            pos_block = (
                header_block.find_all("span", {"class": "pos dpos"})
                if header_block is not None
                else []
            )
            pos = []
            ...
            (
                ...
                pos_usages,
                pos_domains,
            ) = get_tags(header_block)

            for def_and_sent_block in entity.find_all(
                ...
                # sentence examples
                sentences_and_translation_block = def_and_sent_block.find(
                    "div", {"class": "def-body"}
                )
                ...
                if sentences_and_translation_block is not None:
                    definition_translation_block = sentences_and_translation_block.find(
                        ...
                        and any(
                            class_attr == "trans"
                            for class_attr in tag.attrs.get("class", [])
                        )
                    )
                    definition_translation = (
                        definition_translation_block.text
                        if definition_translation_block is not None
                        else ""
                    )
                    sentence_blocks = sentences_and_translation_block.find_all(
                ...
                for item in sentence_blocks:
                    ...
                    examples_translations.append(
                        sent_translation.text if sent_translation is not None else ""
                    )

                found_definition_block = def_and_sent_block.find(
                    "div", {"class": "ddef_h"}
                )

                if found_definition_block is not None:
                    found_definition_string = found_definition_block.find(
                        "div", {"class": "def ddef_d db"}
                    )
                    definition = (
                        ""
                        if found_definition_string is None
                        else found_definition_string.text
                    )

                    # Gathering specific tags for every word usage
                    tag_section = found_definition_block.find(
                        "span", {"class": "def-info"}
                    )
                    (
                        current_word_level,
                    ...
                    phrase_block = found_definition_block.find_parent(
                        "div", {"class": "phrase-block"}
                    )
                    if phrase_block is not None:
                        phrase_tags_section = phrase_block.find(
                        ...
                        current_def_block_word = phrase_block.find(
